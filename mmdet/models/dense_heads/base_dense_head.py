...
class BaseDenseHead(BaseModule, metaclass=ABCMeta):
    ...
    def _get_bboxes_single(self,
        ...
        for level_idx, (cls_score, bbox_pred, score_factor, priors) in \
            ...
            # self.pre_nms_cls_score_value.append(scores)
    ...
    def _bbox_post_process(self,
        ...
        self.post_nms_cls_score_value = []
        self.pre_nms_cls_score_value = []
        self.post_nms_det_bbox_value = []
        self.post_nms_label_value = []

        if with_nms:
            if mlvl_bboxes.numel() == 0:
                self.pre_nms_cls_score_value.append(mlvl_scores)
                det_bboxes = torch.cat([mlvl_bboxes, mlvl_scores[:, None]], -1)
                self.post_nms_cls_score_value.append(det_bboxes[:, -1])
                self.post_nms_det_bbox_value.append(det_bboxes[:, :4])
                self.post_nms_label_value.append(mlvl_labels)
            ...
            self.post_nms_cls_score_value.append(det_bboxes[:, -1])
            self.post_nms_det_bbox_value.append(det_bboxes[:, :4])
            self.post_nms_label_value.append(det_labels)
